...
# Assuming your data is loaded dynamically from the Shiny UI (better for deployment)
data <- read.csv("C:\Users\Hp\Desktop\Pramod_Giri__Student_id_23189635_Data_Visualization\Intersteller\Dashboard\Intersteller.csv")
...
server <- function(input, output) {
  ...
  output$contour_alpha_delta <- renderPlotly({
    p <- ggplot(data_clean, aes(x = alpha, y = delta)) +
      geom_density_2d() +
      theme_minimal() +
      ggtitle("Contour Plot of Alpha vs Delta")
    ggplotly(p)
  })
